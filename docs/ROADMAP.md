...
subcommands — `version`, `status`, `check`, `run`, `test`, `capture` and `project` — and
nothing beyond them. Several v0.2 items are done and are marked so below; the rest of v0.2
is future tense as well.
...
- `[x]` **PLCopen XML**, read and written, with a **compatibility matrix generated by
  ...
  The facts this needs are now established and are in
  `docs/adr/ADR-0003-plcopen-xml-canonical.md`: salman targets v2.01, will not ship a copy of
  the schema because no redistribution grant exists, and will not claim conformance because
  no conformance programme exists. The matrix itself is **blocked on a fixture corpus that
  does not exist with clean provenance** — the same problem the capture fixtures had, and it
  will get the same answer, which is that salman generates what it can and cites what it
  cannot. Until then there is no matrix, and a matrix defaulting to "not tested" is what
  there will be first.
