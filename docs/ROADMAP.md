...
- `[~]` **PLCopen XML**. **Reading** works: v2.01 and v2.0, both families of Structured Text
  wrapper, and a body in a language salman does not read is named rather than dropped.
  Verified against a real CODESYS V3.5 export that PLCopen itself publishes. **Writing does
  not exist yet**, and without it there is no round trip and therefore nothing to build a
  compatibility matrix from.
