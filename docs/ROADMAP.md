...
The capability registry currently holds forty-four entries: forty-one `[x]` and three `[~]`.
The workspace test suite passes; at the time of writing `cargo test --workspace` runs 1179
tests. Both numbers are a snapshot of a moving tree — the registry, not this page, is the
...
The first version that leaves the process. Every item below is in the tree; what is not done
inside each one is said inside it.
...
- `[x]` **The first diagnostics**: findings that carry evidence, say how sure salman is, and
  say *why* when they are not an assertion of fault. Fifteen ship. Four of the eleven groups
  a finding can be in exist so salman can describe its own limits rather than the wire's,
  and a finding in one of them says so when it is rendered.
- `[x]` **PLCopen XML**, read and written, with a **compatibility matrix generated by
  round-tripping every construct** rather than written by hand:
  [`docs/PLCOPEN_COMPATIBILITY.md`](PLCOPEN_COMPATIBILITY.md). v2.01 and v2.0, both families
  of Structured Text wrapper, and a body in a language salman does not read is named rather
  than dropped. Verified against a real CODESYS V3.5 export that PLCopen itself publishes.
  The matrix says plainly what it cannot tell you: it is salman against salman, because no
  corpus of real vendor exports exists with provenance clean enough to commit.

  `docs/adr/ADR-0003-plcopen-xml-canonical.md` records the rest: salman targets v2.01, will
  not ship a copy of the schema because no redistribution grant exists, and will not claim
  conformance because no conformance programme exists.
