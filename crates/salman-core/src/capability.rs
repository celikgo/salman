...
pub static REGISTRY: &[Capability] = &[
    ...
    Capability {
        ...
        note: "v2.01 and v2.0, and a document says which it was — salman modelled v2.01 and \
               ...
               body goes missing between the model and the report.",
    },
    Capability {
        id: "io.plcopen.write",
        area: "Interchange",
        title: "Writing PLCopen XML, and a compatibility matrix generated by trying things",
        status: Status::ImplementedTested,
        milestone: "v0.2",
        evidence: &[
            Evidence {
                file: "crates/salman-plcopen/tests/roundtrip.rs",
                test: "a_document_read_written_and_read_again_is_the_same_document",
            },
            Evidence {
                file: "crates/salman-plcopen/tests/roundtrip.rs",
                test: "the_coordinate_info_the_schema_requires_is_written_even_for_structured_text",
            },
            Evidence {
                file: "crates/salman-plcopen/tests/compat.rs",
                test: "the_committed_matrix_matches_what_the_code_does_now",
            },
            Evidence {
                file: "crates/salman-plcopen/tests/compat.rs",
                test: "a_language_salman_does_not_read_is_not_marked_as_round_tripping",
            },
        ],
        note: "docs/PLCOPEN_COMPATIBILITY.md is generated by round-tripping every construct \
               at the moment it is written, and a test fails if the committed copy disagrees \
               — so it cannot be edited into saying something flattering and cannot drift. \
               Rows where salman's model round-trips and the **document** does not are \
               marked from what salman knows it discarded, not from the comparison: a Ladder \
               body thrown away on the way in comes back looking perfect. The matrix says \
               plainly that it is salman against salman, that salman does not validate \
               against a schema it does not ship, and that no conformance is claimed because \
               PLCopen runs no conformance programme for this format.",
    },
    Capability {
